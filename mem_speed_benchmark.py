...
parser.add_argument('--data', metavar='DIR', default='/data/guanchu/imagenet/',
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.debug_option == "memory":
        print("========== Model Only ===========")
        usage = get_memory_usage(True, args.gpu)
...
def dataloader_init(args):
    ...
    val_data_num = 50000 # len(datasets.ImageFolder('/home/gw22/data/ImageNet/val'))
    ...
    train_loader = torch.utils.data.DataLoader(
        train_dataset_lmdb, batch_size=args.batch_size, shuffle=True,
...
#
#     traindir = '/home/gw22/data/ImageNet/train'
#     valdir = '/home/gw22/data/ImageNet/val'
#
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        if args.debug_option == "memory":
            ...
            init_mem = get_memory_usage(True, args.gpu)
            exp_recorder.record("data_loader", init_mem / GB - exp_recorder.val_dict['model_only'], 2)

        if args.gpu is not None:
            images = images.cuda(args.gpu, non_blocking=True)
            # images = torch.rand(args.batch_size, 3, 224, 224, device=torch.device("cuda:"+str(args.gpu)))
        ...
        if args.debug_option == "memory":
            ...
            after_backward = get_memory_usage(True, args.gpu)
        ...
        if args.debug_option == "speed":
            ...
            if train_step_ct >= MAX_STEP:
                print(train_ips_list)
                train_ips = np.median(train_ips_list) # np.mean(train_ips_list) # np.median(train_ips_list)
                ...
                exp_recorder.dump(os.path.join(args.outputdir, args.log_fname)) # 'speed_results.json')
                exit(0)
            train_step_ct += 1
