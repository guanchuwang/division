...
parser = argparse.ArgumentParser(description='DIVISION cifar100')
...
parser.add_argument('-hf', '--half_precision', dest='amp', action='store_true')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    train_dataset = datasets.CIFAR100(root=args.data_path, train=True, download=True, transform=transform_train)
    val_dataset = datasets.CIFAR100(root=args.data_path, train=False, download=True, transform=transform_val)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        #                             str(args.bn_window_size) + "_checkpoint"
        #     save_checkpoint({
        #         'epoch': epoch + 1,
        #         'arch': args.arch,
...
def scheduler_init(optimizer, args):
    ...
    if args.scheduler == "warmup_coslr":
        ...
        milestones = [30, 60, 90] if args.epochs == 100 else [60, 120, 160]
