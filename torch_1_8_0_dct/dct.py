...
def dct(x, norm=None):
    ...
    V_complex = torch.fft.fft(v)
    Vc = torch.cat((V_complex.real.unsqueeze(dim=2), V_complex.imag.unsqueeze(dim=2)), dim=2)
...
def idct(X, norm=None):
    ...
    V_complex = torch.complex(V_r, V_i)
    v = torch.fft.ifft(V_complex).real
