...
class FDMP_(Function):
    ...
    def freq_divide(x, dct_op, window_size, device=None):
        ...
        x_lfc_dct = dct_op(dct_matrix, x, block_num)
        x_lfc = dct_op(dct_matrix, x_lfc_dct, block_num, inverse=True)
...
class WDCT(Function):
    ...
    def dct_2d_(dct_matrix, x, B=1):
        ...
        x = x.view((n2*B, n2*B, x_shape[0], x_shape[1])).permute(2, 3, 0, 1)

        return x

    @staticmethod
    @torch.no_grad()
    def idct_2d_(dct_matrix, x_dct, B=1):
        return WDCT.dct_2d_(dct_matrix.T, x_dct, B)

    @staticmethod
    @torch.no_grad()
    def dct_2d(dct_matrix, x, B=1, inverse=False):

        if not inverse:
            return WDCT.dct_2d_(dct_matrix, x, B)

        else:
            return WDCT.idct_2d_(dct_matrix, x, B)
