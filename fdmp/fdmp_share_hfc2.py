...
class FDMP_(Function):
    ...
    def freq_divide(x, dct_op, window_size, device=None):
        ...
        return x_lfc_dct, x_hfc_dct # x - x_lfc_mean # x_mean - x_lfc_mean

    @staticmethod
    @torch.no_grad()
    def fdmp_simulation(x, dct_op, window_size, device):

        x_lfc_dct, x_hfc_dct = FDMP.freq_divide(x, dct_op, window_size, device=device)
        return x_lfc_dct, x_hfc_dct
    ...
    def de_fdmp_simulation(feature_pack, dct_op, q_input_shape, window_size, device):
        ...
        x_lfc_dct, x_hfc_dct = feature_pack

        N = q_input_shape[-1]
        n = abs_window_size(N, window_size)

        x_dct = x_hfc_dct.repeat((1, x_lfc_dct.shape[1], 1, 1))
        x_dct[:, :, 0:n, 0:n] = x_lfc_dct

        dct_matrix = WDCT.generate_dct_matrix(N, N, device)
        x = dct_op(dct_matrix, x_dct, inverse=True)
